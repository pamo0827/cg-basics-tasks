...
# 球体の置かれている床
g_Board = Board(-150)	# y座標値を -150 にする。

# 床の色を計算する関数
def getFloorColor(intersection):
	# 床の色を取得（格子模様）
	...
	shadow_ray_origin = intersection + 0.001 * light_dir
...
def getPixelColor(x, y):
	...
	if t > 0.0:	# 球との交点がある
		...
		# 【発展課題2】球面での反射を計算
		# 入射レイを法線で反射させた反射レイを計算
		# 反射レイ = 入射レイ - 2(入射レイ・法線)法線
		reflect_ray = ray - 2.0 * ray.dot(normal) * normal
		reflect_ray = normalize(reflect_ray)

		# 反射レイの始点（微小量だけずらす）
		reflect_origin = intersection + 0.001 * reflect_ray

		# 反射レイが床と交差するかチェック
		t_floor = g_Board.getIntersect(reflect_origin, reflect_ray)

		if t_floor > 0.0:  # 床と交差する
			# 床との交点を計算
			floor_intersection = reflect_origin + t_floor * reflect_ray

			# 床の色を取得
			floor_color = getFloorColor(floor_intersection)

			# 球の色と床の色を足して2で割る（PDFの指示通り）
			reflection_color = (I + floor_color) / 2.0

			# 反射の重みを考慮して最終色を決定
			I = (1.0 - g_ReflectionWeight) * I + g_ReflectionWeight * reflection_color
	...
	if t > 0.0:	# 床との交点がある
		# 交点の座標を計算
		intersection = g_Viewpoint + t * ray
		return getFloorColor(intersection)
...
if __name__ == "__main__":
	...
	g_WindowID = glutCreateWindow("発展課題(2): 球面での鏡面反射")
