...
# 複数の球体を配置
g_Spheres = [
	Sphere(vec3(-200, 0, -1500.), 100.0, vec3(0.2, 0.9, 0.9)),  # 左の青緑の球
	Sphere(vec3(200, 0, -1500.), 100.0, vec3(0.9, 0.2, 0.9)),   # 右のマゼンタの球
	Sphere(vec3(0, 0, -1800.), 80.0, vec3(0.9, 0.9, 0.2)),      # 奥の黄色の球
]
...
# 球体の色を計算する関数
def getSphereColor(sphere, intersection, ray):
...
def getPixelColor(x, y):
	...
	for sphere in g_Spheres:
		t = sphere.getIntersect(g_Viewpoint, ray)
		if t > 0.0 and t < min_t:
			min_t = t
			hit_sphere = sphere

	if hit_sphere is not None:	# 球との交点がある
		intersection = g_Viewpoint + min_t * ray
		return getSphereColor(hit_sphere, intersection, ray)
	...
	if t > 0.0:	# 床との交点がある
		...
		# 影の判定：交点から光源方向にレイを飛ばして全ての球と交わるかチェック
		shadow_ray_origin = intersection + 0.001 * light_dir
		in_shadow = False

		for sphere in g_Spheres:
			t_shadow = sphere.getIntersect(shadow_ray_origin, light_dir)
			if t_shadow > 0.0:
				in_shadow = True
				break
...
if __name__ == "__main__":
	...
	g_WindowID = glutCreateWindow("発展課題(1): 複数の球")
